...
let earthPos = p.createVector(140, 520);
let ec = p.color(0, 204, 200); // earth color, blue

let pos = p.createVector(220, 500);
let vel = p.createVector(0, -1.3);
...
p.draw = function() {
    ...
    p.fill(barC);
    p.rect(20, 45, 800, 50);
    p.rect(20, 160, 800, 50);
    // Filled bars
    p.fill(potC);
    p.rect(20, 45, 800 * fuel/maxFuel, 50);
    p.fill(kinC);
    p.rect(20, 160, 400 + energy, 50);
    p.fill("black");
    p.text(energy.toFixed(2), 715, 193);
    // Zero border
    p.fill("yellow");
    p.rect(418, 160, 4, 50);
    // Text
    ...
    p.fill(233, 233, 233);
    p.text('Fuel', 390, 30);
    p.text('Mechanical Energy', 305, 140);
};
...
function boost(vel) {
    ...
    if (p.mouseIsPressed && p.mouseX > 0 && 
        p.mouseY > 0 && p.mouseX < 1080 && p.mouseY < 720 &&
        fuel > 0) {
        bst = p5.Vector.normalize(vel).mult(0.010);
        fuel -= 1;
    } else {
...
function resetSketch() {
    pos = p.createVector(220, 500);
    vel = p.createVector(0, -1.3);
    history = [];
    fuel = maxFuel;
    energy = mechanical(earthPos, pos, vel);
};
